...
namespace Morpheus { namespace Vulkan {
	...
	// Device = 0, Unit = 1, Pipeline = 2, VertexArray = 3
	// VertexBuffer = 4, IndexBuffer = 5, VertexBuffer = 6, VertexBuffer = 7
	...
	public:
		...
		Ref<T> Get(const uint32& _ID)
		{
			...
			if (it != m_Cache.end())
				return it->second;
			VULKAN_CORE_ASSERT(VULKAN_ERROR, "[VULKAN] Could not find value #" + ToString(_ID) + " in Cache!");
			return nullptr;
		}
